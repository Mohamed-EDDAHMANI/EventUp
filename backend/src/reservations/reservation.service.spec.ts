...
describe('ReservationsService', () => {
  ...
  let reservationModel: { findOne: jest.Mock; findById: jest.Mock; findByIdAndUpdate: jest.Mock };
  let eventsService: { findOne: jest.Mock; reservePlaceIfCapacity: jest.Mock; decrementReservedCount: jest.Mock };
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ReservationsService,
        { provide: getModelToken(Reservation.name), useValue: Object.assign(ModelConstructor, model) },
        { provide: EventsService, useValue: eventsService },
        { provide: PdfTicketService, useValue: { generateTicket: jest.fn().mockResolvedValue(Buffer.from('')) } },
      ],
    }).compile();

    service = module.get<ReservationsService>(ReservationsService);
    reservationModel = module.get(getModelToken(Reservation.name)) as typeof model;
    jest.clearAllMocks();
  });
  ...
  describe('create', () => {
    it('throws if event is not PUBLISHED (DRAFT)', async () => {
      ...
      await expect(service.create(eventId, userId)).rejects.toThrow(BadRequestException);
    });
    ...
    it('throws if participant already has active reservation', async () => {
      eventsService.findOne.mockResolvedValue(event());
      reservationModel.findOne.mockReturnValue({ exec: jest.fn().mockResolvedValue(reservation()) });
      await expect(service.create(eventId, userId)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('adminConfirm', () => {
    it('throws if reservation is not PENDING', async () => {
      reservationModel.findById.mockReturnValue(chain(reservation({ status: 'CONFIRMED' })));
      await expect(service.adminConfirm(reservationId)).rejects.toThrow(BadRequestException);
    });

    it('updates to CONFIRMED when valid', async () => {
      reservationModel.findById.mockReturnValue(chain(reservation({ status: 'PENDING' })));
      reservationModel.findByIdAndUpdate.mockReturnValue(chain(reservation({ status: 'CONFIRMED' })));
  ...
  describe('adminRefuse', () => {
    it('updates to CANCELLED and decrements capacity', async () => {
      reservationModel.findById.mockReturnValue(chain(reservation({ status: 'PENDING' })));
      reservationModel.findByIdAndUpdate.mockReturnValue(chain(reservation({ status: 'CANCELLED' })));
      ...
      expect(eventsService.decrementReservedCount).toHaveBeenCalledWith(eventId);
    });
  });

  describe('cancel', () => {
    it('throws if user is not owner', async () => {
      reservationModel.findById.mockReturnValue(chain(reservation({ user: 'other-user-id' })));
      await expect(service.cancel(reservationId, userId)).rejects.toThrow(ForbiddenException);
    });

    it('throws if already CANCELLED', async () => {
      reservationModel.findById.mockReturnValue(chain(reservation({ status: 'CANCELLED' })));
      await expect(service.cancel(reservationId, userId)).rejects.toThrow(BadRequestException);
    });

    it('updates to CANCELLED and decrements capacity', async () => {
      reservationModel.findById.mockReturnValue(chain(reservation({ status: 'PENDING', user: userId })));
      reservationModel.findByIdAndUpdate.mockReturnValue(chain(reservation({ status: 'CANCELLED' })));
      eventsService.decrementReservedCount.mockResolvedValue(undefined);
